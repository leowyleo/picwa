...
zsh scripts/archive.sh /tmp/Picrow.xcarchive
```
...
- `Sources/PicLook/Timeline.swift`：时间线分组、按需缩略图与 64 MB 内存缓存。
- `Sources/PicLook/Index.swift`：ImageIO 元数据扫描、SQLite 分批事务、路径去重。
- `Sources/PicLook/PicLookApp.swift`：原生界面、目录授权、FSEvents 实时同步与重启恢复。
- `scripts/bundle.sh`、`scripts/PicLook.entitlements`、`PrivacyInfo.xcprivacy`：本地沙盒 App 打包与隐私清单。
- `scripts/VerifyIndex.swift`：真实文件扫描、重复扫描、持久化与无效目录验证入口。
- `docs/ACCEPTANCE.md`：分阶段验收记录。

## 数据位置

`~/Library/Containers/cc.leowy.picrow/Data/Library/Application Support/PicLook/`
