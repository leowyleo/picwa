    ...
    private func addIncludedFolders(_ urls: [URL]) {
        for url in urls {
            ...
            do {
                ...
                let existing = folders.firstIndex { record in
                    if access[record.id]?.standardizedFileURL == url.standardizedFileURL { return true }
                    var stale = false
                    let resolved = try? URL(resolvingBookmarkData: record.bookmark, options: [.withSecurityScope, .withoutUI], relativeTo: nil, bookmarkDataIsStale: &stale)
                    return resolved?.standardizedFileURL == url.standardizedFileURL
                }
                let record = FolderRecord(id: existing.map { folders[$0].id } ?? UUID(), name: url.lastPathComponent, bookmark: bookmark, complete: false)
                if let i = existing { folders[i] = record } else { folders.append(record) }
                do { try save() } catch { if started { url.stopAccessingSecurityScopedResource() }; throw error }
                if let old = access[record.id] { old.stopAccessingSecurityScopedResource() }
            ...
            while let folder = activeRecords.first(where: { !$0.complete && access[$0.id] != nil && !attempted.contains($0.id) }) {
                ...
                do {
                    let result = try await index.scan(url, ignoring: ignoredPaths, visibleFolders: activePaths, batch: { [weak self] snapshot in
                        ...
                        Task { @MainActor in guard let self, generation == self.scanGeneration else { return }; self.status = L.tr("正在索引 \(count) 张图片…", "Indexing \(count) images…") }
                    }
                    if result.errors == 0 {
                    ...
                    if result.errors > 0 { issues.append(L.tr("\(folder.name)：\(result.errors) 处读取失败。", "\(folder.name): \(result.errors) read failures.")) }
